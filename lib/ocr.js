...
/* ── Tesseract local OCR (no API key needed) ── */
async function tryTesseract(filePath) {
  try {
    console.log('🟡 Trying Tesseract local OCR...');
    ...
    console.log(`✅ Tesseract success — ${result.length} chars`);
    return result;
  } catch (err) {
...
/* ── Cloud providers cascade ── */
async function tryCloudProviders(base64, mimeType) {
  // 1. Gemini
...
/* ── Main cascade: cloud + Tesseract race in parallel ── */
async function callAI(base64, mimeType, originalFilePath) {
  // Both start immediately — first to succeed wins
  const cloudPromise = tryCloudProviders(base64, mimeType).then(r => {
    if (r !== null) { console.log('✅ Cloud provider won the race'); return r; }
    throw new Error('all_cloud_failed');
  });

  const tesseractPromise = tryTesseract(originalFilePath).then(r => {
    if (r !== null) { console.log('✅ Tesseract won the race'); return r; }
    throw new Error('tesseract_failed');
  });

  try {
    // Promise.any resolves the moment EITHER succeeds — no waiting for the other
    return await Promise.any([cloudPromise, tesseractPromise]);
  } catch {
    throw new Error('All providers failed. Please top up your API credits or wait for quota reset.');
  }
}
...
async function extractFromFiles(files) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const t1 = Date.now();
      const text = await callAI(base64, mimeType, file.path);
