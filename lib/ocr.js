...
async function callAI(base64, mimeType, originalFilePath) {
  ...
  // 5. Tesseract — local OCR, always available, no API key needed
  const tesseractResult = await tryTesseract(originalFilePath);
  if (tesseractResult !== null) return tesseractResult;
