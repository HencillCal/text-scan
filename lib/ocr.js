...
if (apiKeys.length === 0) {
  console.warn('😡 [OCR] WARNING: No API keys found. Add them to settings.json under gemini.apiKeys');
}

// One client per key — rotated on quota exhaustion
...
async function callGemini(base64, mimeType) {
  ...
  for (const model of MODELS) {
    // Try every key for this model before moving to next model
    let modelWorked = false;

    for (let keyIdx = 0; keyIdx < clients.length; keyIdx++) {
      // Retry up to 3 times for temporary errors on this key+model
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          console.log(`✅ model=${model} key=#${keyIdx + 1}/${clients.length} attempt=${attempt}`);
          ...
          console.log(`✅ Success — ${text.length} chars extracted`);
          return text;
        } catch (err) {
          lastErr = err;

          if (isNotFound(err)) {
            console.warn(`😡 model=${model} not available — skipping all keys for this model`);
            modelWorked = false;
            // Break both key and attempt loops — model doesn't exist
            keyIdx = clients.length;
            break;
          }
          ...
          if (isRetryable(err)) {
            if (attempt < 3) {
              ...
              console.warn(`😡 model=${model} key=#${keyIdx + 1} overloaded — retrying in ${delay}ms...`);
              await sleep(delay);
              continue;
            }
            console.warn(`😡 model=${model} key=#${keyIdx + 1} still overloaded — trying next key...`);
            break;
          }

          if (isQuota(err)) {
            console.warn(`😡 model=${model} key=#${keyIdx + 1} quota hit — trying next key...`);
            break; // try next key
          }

          // Unknown error — log and try next key
          console.warn(`😡 model=${model} key=#${keyIdx + 1} error: ${err.message} — trying next key...`);
          break;
        }
      }
    }
